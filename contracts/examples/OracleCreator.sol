...
contract OracleCreator {    
    ...
    function createOracle(
      ...
      emit OracleCreated(oracleId, pair, windowSize, granularity, periodSize, executionBountyWei);
      oraclesIndex++;
    }

    function update(uint256 _oracleId) public payable {
        require(_oracleId <= oraclesIndex, 'DXliquidityRelay: INVALID_ORACLEID');
        require(
            oracles[_oracleId].observationFinished == false,
            'DXliquidityRelay: OBSERVATION_FINISHED'
        );

        address pair = oracles[_oracleId].pair;

        // populate the array with empty observations (first call only)
        for (uint256 i = oracleObservations[_oracleId].length; i < oracles[_oracleId].granularity; i++) {
            oracleObservations[_oracleId].push();
        }

        // get the observation for the current period
        uint8 observationIndex = observationIndexOf(_oracleId, block.timestamp);
        Observation storage observation = oracleObservations[_oracleId][observationIndex];

        // we only want to commit updates once per period (i.e. windowSize / granularity)
        uint256 timeElapsed = block.timestamp - observation.timestamp;
        if (timeElapsed > oracles[_oracleId].periodSize) {
        ...
        if (observationIndex == oracleObservations[_oracleId].length) {
            oracles[_oracleId].observationFinished = true;
            oracles[_oracleId].finishedTimestamp = block.timestamp;
            emit OracleExecuted(_oracleId);
        }

        // send the bounty to msg.sender
        TransferHelper.safeTransferETH(msg.sender, oracles[_oracleId].executionBountyWei);
    }
    ...
    function getFirstObservationInWindow(uint256 _oracleId)
        private
        view
        returns (Observation storage firstObservation)
    {
        ...
        uint8 firstObservationIndex = (observationIndex + 1) % oracles[_oracleId].granularity;
        firstObservation = oracleObservations[_oracleId][firstObservationIndex];
    }
    ...
    function consult(
        ...
        address pair = oracles[_oracleId].pair;
        Observation storage firstObservation = getFirstObservationInWindow(_oracleId);

        uint256 timeElapsed = block.timestamp - firstObservation.timestamp;
        require(timeElapsed <= oracles[_oracleId].windowSize, 'DXliquidityRelay: MISSING_HISTORICAL_OBSERVATION');
        // should never happen.
        require(timeElapsed >= oracles[_oracleId].windowSize - oracles[_oracleId].periodSize * 2, 'DXliquidityRelay: UNEXPECTED_TIME_ELAPSED');
